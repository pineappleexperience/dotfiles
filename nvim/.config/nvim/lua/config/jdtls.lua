-- ### Java LSP support ###
-- including Lombok
local function get_mason_pckg_path(name)
	local path = string.format("$MASON/packages/%s", name)
	return vim.fn.expand(path)
end

local function get_jdtls()
	local jdtls_path = get_mason_pckg_path("jdtls")
...
local function get_bundles()
	...
	local java_debug_path = get_mason_pckg_path("java-debug-adapter")
	...
	local java_test_path = get_mason_pckg_path("java-test")
...
local function setup_jdtls()
	...
	if vim.fn.filereadable(lombok) == 0 then
		notify("Lombok jar not found at: " .. lombok, vim.log.levels.ERROR)
	else
		notify("Lombok jar found at: " .. lombok, vim.log.levels.INFO)
	end

	-- Get the path you specified to hold project information
	local workspace_dir = get_workspace()

	notify("Workscpace at: " .. workspace_dir, vim.log.levels.INFO)
